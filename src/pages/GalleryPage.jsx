...
import photosData from '../data/galleryData.json';
...
function GalleryPage() {
    // 2. Langsung gunakan data yang diimpor untuk mengisi state
    // .reverse() agar data terbaru (yang Anda tambahkan di akhir file) muncul pertama
    const [photos] = useState([...photosData].reverse());
    const [selectedPhoto, setSelectedPhoto] = useState(null);

    // Kita tidak lagi butuh state 'loading' dan 'error'
    // Kita juga tidak lagi butuh 'useEffect' untuk fetch data
    ...
    // Tampilan loading dan error bisa dihapus karena data dimuat secara instan
        ...
        <div className="py-20 md:py-28">
            ...
            <div className="container mx-auto max-w-7xl px-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                    {photos.map((photo) => (
                        <PhotoCard
                            key={photo.id} // 3. Gunakan 'id' dari JSON sebagai key
                            photo={photo}
                            onClick={() => handlePhotoClick(photo)}
                        />
                    ))}
